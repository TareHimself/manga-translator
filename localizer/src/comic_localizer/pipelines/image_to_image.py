...
import cv2
import asyncio
from comic_localizer.core.typing import Vector4i
from comic_localizer.utils import compute_draw_bbox, perf_async
...
class FrameSection:
    def __init__(
        ...
        source: np.ndarray,
        section: np.ndarray,
        cleaned_section: np.ndarray,
        mask: np.ndarray,
        text: np.ndarray,
        bbox: Vector4i,
        draw_section: np.ndarray,
        draw_bbox: Vector4i,
    ):
...
class ImageToImagePipeline(Pipeline):
        ...
        for result in detection:
            ...
            sections.append(
                FrameSection(
                    ...
                    frame,
                    frame[y1:y2, x1:x2],
                    cleaned_frame[y1:y2, x1:x2],
                    mask[y1:y2, x1:x2],
                    text_only[y1:y2, x1:x2],
                    result.bbox.copy(),
                    cleaned_frame[dy1:dy2, dx1:dx2],
                    draw_bbox,
                )
            )
    ...
    async def extract_detected_sections_batched(
        ...
        result = []
        for sections in await asyncio.gather(
            *[
                asyncio.to_thread(
                    self.extract_detected_sections,
                    x,
                    frame,
                    cleaned_frame,
                    mask,
                ...
                for x, frame, cleaned_frame, mask, detection in zip(
                    range(len(frames)), frames, cleaned_frames, masks, detections
                )
            ]
        ):
            result += sections
    ...
    def clean_frame_using_masks_and_detections(
        self,
        frame: np.ndarray,
        cleaned_frame: np.ndarray,
        mask: np.ndarray,
    ...
    async def clean_frames_using_masks_and_detections(
        ...
        return await asyncio.gather(
            *[
                asyncio.to_thread(self.clean_frame_using_masks_and_detections, *args)
                for args in zip(frames, cleaned_frames, masks, detections)
            ]
        )

    def composite_drawn_sections(
        self, sections: list[tuple[FrameSection, np.ndarray, np.ndarray]]
    ):
    ...
    async def __call__(self, batch: list[np.ndarray]) -> list[np.ndarray]:
        # """
        # Expects all images in batch to be RGB
        ...
        detections = await self.detector(batch)
        ...
        for i in range(len(batch)):
            ...
            translatable_frames.append(TranslatableFrame(i, frame, detections[i], []))
        ...
        masks = await asyncio.gather(
            *[
                asyncio.to_thread(self.make_mask, item.frame, item.segments)
                for item in translatable_frames
            ]
        )

        # Produce cleaned frames
        cleaned_frames = await self.cleaner(
            raw_translatable_frames, masks, segments, detections
        )

        final_cleaned_frames = await self.clean_frames_using_masks_and_detections(
            raw_translatable_frames, cleaned_frames, masks, detections
        )
        ...
        sections = await self.extract_detected_sections_batched(
            raw_translatable_frames, cleaned_frames, masks, detections
        )
        ...
        valid_ocr_indices = [
            i for i in range(len(ocr_results)) if len(ocr_results[i].text.strip()) > 0
        ]
        ...
        if len(valid_ocr_indices) > 0:
            ...
            valid_translation_indices = [
                ...
                if len(translation_results[i].text.strip()) > 0
            ]
            ...
            if len(valid_translation_indices) > 0:
                if resize_sections:
                    x = [
                        sections[valid_ocr_indices[i]]
                        for i in valid_translation_indices
                    ]
                    sections = x
                ...
                drawn_results = await self.drawer(
                    [section.draw_section for section in sections],
                    translation_results,
                ...
                for section, drawn_result in zip(sections, drawn_results):
                    frame_sections[section.source_index].append(
                        (section, *drawn_result)
                    )

                await asyncio.gather(
                    *[
                        asyncio.to_thread(
                            self.composite_drawn_sections, frame_sections[i]
                        )
                        for i in range(len(raw_translatable_frames))
                    ]
                )
