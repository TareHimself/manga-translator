...
def get_color_detection_model(
        build_final_layer: Callable[[int], nn.Module] = lambda a: nn.Sequential(nn.Dropout(.4), nn.Linear(a, 128),
                                                                                nn.ReLU(), nn.Dropout(.4),
                                                                                nn.Linear(128, 3), nn.Sigmoid()),
        channels=3,
        model_name="efficientnet_b4", pretrained=False, device=torch.device('cpu'), weights_path=None):
